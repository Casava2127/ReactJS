...
[<< Day 7](../07_Day_Class_Components/07_class_components.md) | [Day 9 >>](../09_Day_Conditional_Rendering/09_conditional_rendering.md)
Đúng vậy, bạn đã hiểu đúng về state trong React. Dưới đây là một phân tích chi tiết về state và cách nó hoạt động:

### State trong React
1. **State là một object**: Nó lưu trữ dữ liệu động của component. Mỗi component có thể có một state riêng để quản lý dữ liệu của chính nó.
2. **Khai báo state ban đầu**: State được khai báo trong constructor hoặc trực tiếp trong class component bằng cách sử dụng cú pháp mới (không cần constructor).
3. **Thay đổi state bằng `setState`**: Để thay đổi state, chúng ta sử dụng phương thức `setState`. Phương thức này nhận vào một đối tượng chứa các giá trị mới của state.

### Khi state thay đổi:
1. **Cập nhật state**: Khi `setState` được gọi, React sẽ hợp nhất đối tượng mới với state hiện tại.
2. **So sánh DOM ảo và DOM thật**: React sử dụng thuật toán so sánh (reconciliation) để so sánh DOM ảo (virtual DOM) với DOM thật. DOM ảo là một bản sao nhẹ của DOM thật được lưu trữ trong bộ nhớ.
3. **Cập nhật giao diện**: Nếu có sự khác biệt giữa DOM ảo và DOM thật, React sẽ cập nhật DOM thật để phản ánh các thay đổi. Quá trình này gọi là "re-render".

### Ví dụ minh họa:
Giả sử bạn có một component với state và phương thức để thay đổi state:
...
class App extends React.Component {
  ...
  // Phương thức để tăng giá trị count
  addOne = () => {
    this.setState({ count: this.state.count + 1 });
  };

  // Phương thức để giảm giá trị count
  minusOne = () => {
    this.setState({ count: this.state.count - 1 });
  };

  render() {
    // Lấy giá trị count từ state
    const count = this.state.count;
    return (
      <div className="App">
        <h1>{count}</h1>
...
### Trình tự chạy:
1. **Render ban đầu**: Khi component được render lần đầu tiên, React sẽ tạo một DOM ảo dựa trên JSX được trả về từ phương thức `render`.
2. **Thay đổi state**:
   - Khi bạn nhấn nút "+1", phương thức `addOne` được gọi và `setState` được kích hoạt.
   - React cập nhật state với giá trị mới (`count + 1`).
3. **So sánh và cập nhật DOM**:
   - React so sánh DOM ảo mới (với giá trị `count` mới) với DOM ảo cũ.
   - Nếu có sự khác biệt, React sẽ cập nhật DOM thật để phản ánh sự thay đổi này.
4. **Re-render**: Component sẽ được re-render với state mới, và giao diện sẽ được cập nhật.

### Tổng kết:
State là một đối tượng quản lý dữ liệu động của component. Khi state thay đổi, React sẽ tự động cập nhật giao diện bằng cách so sánh DOM ảo và DOM thật, sau đó chỉ cập nhật những phần cần thiết để tối ưu hiệu suất.