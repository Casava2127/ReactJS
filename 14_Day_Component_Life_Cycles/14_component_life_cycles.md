...
// chạy hàm contructors -> chạy hàm render() -> chạy hàm componentDidMount -> state thay đổi nên chạy lại render()
const rootElement = document.getElementById('root')
ReactDOM.render(<App />, rootElement)
```
...
ReactDOM.render(<App />, rootElement)
```
// chạy như sau, ban đầu khởi tạo contrucotrs, state, render lần đầu, khi nhấn button thì hàm doChange đc gọi, lúc này state chưa đc render ra mà chạy lại hàm hàm should để kiểm tra xem có nên reander, nếu true thì gọi hàm render() -> gọi hàm update, nếu không thi không cần render -> không gọi update
